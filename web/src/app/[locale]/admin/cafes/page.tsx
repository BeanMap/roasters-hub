...
export default async function AdminCafesPage({ searchParams }: { searchParams: SearchParams }) {
  ...
  const where = {
    ...(statusFilter !== "ALL" ? { status: statusFilter as CafeStatus } : {}),
    ...
    <>
      ...
      <main className="max-w-7xl mx-auto px-6 py-12">
        <div className="flex justify-between items-end mb-8">
          <div>
            <h1 className="font-headline text-4xl font-bold text-on-background mb-2">{t("cafes")}</h1>
        ...
        {/* Search */}
        <form action="/admin/cafes" method="get" className="mb-4">
          ...
          <div className="flex gap-2">
            ...
            {q && (
              <Link
                href={`/admin/cafes${buildQuery({ q: "", page: 1 })}`}
        ...
        <div className="flex flex-wrap items-center gap-3 mb-6">
          ...
          {STATUS_OPTIONS.map((s) => (
            <Link
              key={s}
              href={`/admin/cafes${buildQuery({ status: s, page: 1 })}`}
          ...
          <Link
            href={`/admin/cafes${buildQuery({ sort: "newest", page: 1 })}`}
          ...
          >{t("newest")}</Link>
          <Link
            href={`/admin/cafes${buildQuery({ sort: "oldest", page: 1 })}`}
        ...
        <div className="bg-surface-container-lowest rounded-xl overflow-hidden editorial-shadow">
          <table className="w-full text-sm">
            ...
            <tbody>
              {cafes.map((c) => (
                <tr key={c.id} className="border-t border-outline-variant/20 hover:bg-surface-container-lowest/50">
                  <td className="px-5 py-3">
                    <Link href={`/admin/cafes/${c.id}`} className="font-bold text-primary hover:underline">{c.name}</Link>
                  </td>
                  <td className="px-5 py-3 text-on-surface-variant">{c.city}, {c.country}</td>
                  <td className="px-5 py-3">
                    <span className={`inline-block px-2 py-0.5 rounded text-xs font-bold ${STATUS_COLORS[c.status]}`}>{c.status}</span>
                  </td>
                  <td className="px-5 py-3 text-on-surface-variant">{c._count.events}</td>
                  <td className="px-5 py-3 text-on-surface-variant">{c.updatedAt.toLocaleDateString()}</td>
                  <td className="px-5 py-3 text-on-surface-variant">{c.createdAt.toLocaleDateString()}</td>
                </tr>
              ))}
              {cafes.length === 0 && (
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-3 mt-6">
            {page > 1 && (
              <Link href={`/admin/cafes${buildQuery({ page: page - 1 })}`} className="px-4 py-2 rounded-lg bg-surface-container-low font-bold hover:bg-surface-container">{t("prev")}</Link>
            )}
            <span className="text-sm text-on-surface-variant">{t("total", { total: page, page, totalPages })}</span>
            {page < totalPages && (
              <Link href={`/admin/cafes${buildQuery({ page: page + 1 })}`} className="px-4 py-2 rounded-lg bg-surface-container-low font-bold hover:bg-surface-container">{t("next")}</Link>
            )}
          </div>
        )}
      </main>
